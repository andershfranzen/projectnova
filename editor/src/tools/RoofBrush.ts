...
/**
 * Roof brush tool — click tiles to paint/remove roofs.
 * Shift+click to remove.
 */
export class RoofBrush implements EditorToolInterface {
  ...
  private apply(wx: number, wz: number, ctx: EditorToolContext): void {
    ...
    if (this.shiftDown) {
      ctx.stateMgr.removeRoof(tx, tz);
    } else {
      ctx.stateMgr.setRoof(tx, tz, {
        height: s.roofHeight,
        style: s.roofStyle,
        peakHeight: s.roofStyle !== 'flat' ? s.roofPeakHeight : undefined,
      });
    }
    ctx.requestRender();
  }
